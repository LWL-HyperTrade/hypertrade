...
import {BuilderPadLauncherToken} from "./BuilderPadLauncherToken.sol";
import {BuilderPadBondingCurve} from "./BuilderPadBondingCurve.sol";
import {BuilderPadBuybackVault} from "./BuilderPadBuybackVault.sol";
import {BuilderPadLaunchLocker} from "./BuilderPadLaunchLocker.sol";
import {BuilderPadMemeHook} from "./hooks/BuilderPadMemeHook.sol";
import {BuilderPadGraduationExecutor} from "./BuilderPadGraduationExecutor.sol";
import {LaunchDeployment, BuilderPadLaunchDeployer} from "./BuilderPadLaunchDeployer.sol";
import {BuilderPadGraduationGuard} from "./BuilderPadGraduationGuard.sol";
import {BuilderPadGraduationMath} from "./libraries/BuilderPadGraduationMath.sol";
import {BuilderPadBondingCurveMath} from "./libraries/BuilderPadBondingCurveMath.sol";
import {
    FeePolicySnapshot,
    GraduationPhase,
    IBuilderPadFeeEscrow,
    IBuilderPadLaunchFactory
} from "./interfaces/ILaunchpadV2.sol";

/**
 * @title BuilderPadLaunchFactory
 ...
 * Uniswap V4 position governed by the shared BuilderPadMemeHook.
 *
 ...
 */
contract BuilderPadLaunchFactory is Ownable2Step, ReentrancyGuard, IBuilderPadLaunchFactory {
    ...
    // Mirrors BuilderPadGraduationGuard's own ceiling.
    ...
    struct TokenParams {
        ...
        string logo;
        string description;
        BuilderPadLauncherToken.Socials socials;
        ...
        // BuilderPadLaunchDeployer.predictLaunchAddresses to check in advance.
        bytes32 salt;
    }
    ...
    BuilderPadLaunchLocker public immutable locker;
    BuilderPadMemeHook public immutable memeHook;
    IBuilderPadFeeEscrow public immutable feeEscrow;
    BuilderPadBuybackVault public immutable buybackVault;
    ...
    BuilderPadGraduationExecutor public graduationExecutor;
    BuilderPadLaunchDeployer public launchDeployer;
    address public launchForwarder;
    BuilderPadGraduationGuard public immutable graduationGuard;
    ...
    constructor(
        ...
        BuilderPadLaunchLocker locker_,
        BuilderPadMemeHook memeHook_,
        IBuilderPadFeeEscrow feeEscrow_,
        BuilderPadBuybackVault buybackVault_,
        ...
        graduationGuard = new BuilderPadGraduationGuard();
     ...
     * routers like BuilderPadLaunchAndBuy can hold their own callers to this
     ...
     */
    function setGraduationExecutor(BuilderPadGraduationExecutor executor) external onlyOwner {
     ...
     * already-known address.
     */
    function setLaunchDeployer(BuilderPadLaunchDeployer deployer) external onlyOwner {
    ...
    function _exemptFromSnipeTax(address curve, address[] calldata snipeTaxExemptions) private {
        ...
        for (uint256 i = 0; i < snipeTaxExemptions.length; ++i) {
            BuilderPadBondingCurve(curve).exemptFromSnipeTax(snipeTaxExemptions[i]);
        }
    }
    ...
    function _launchToken(
        ...
        BuilderPadBondingCurve(curve).initialize(token);
        ...
        BuilderPadBondingCurve(curve).exemptFromSnipeTax(originalDeployer);
        if (creatorFeeRecipient != originalDeployer) {
            BuilderPadBondingCurve(curve).exemptFromSnipeTax(creatorFeeRecipient);
        }
    ...
    function setBuybackEnabled(address token, bool enabled) external {
        ...
        if (launch.phase == GraduationPhase.NotGraduated) {
            BuilderPadBondingCurve(launch.curve).setBuybackEnabled(enabled);
    ...
    function _setCreatorFeeRecipient(address token, LaunchedToken storage launch, address newRecipient) private {
        ...
        if (launch.phase == GraduationPhase.PoolCreated) {
            ...
            BuilderPadBondingCurve(launch.curve).setCreatorFeeRecipient(newRecipient);
        }
    ...
    function graduate(address token) external nonReentrant {
        ...
        BuilderPadBondingCurve curve = BuilderPadBondingCurve(launch.curve);
        if (!curve.readyToGraduate()) {
            revert BuilderPadBondingCurve.NotReadyToGraduate();
        }
     ...
     */
    function _sweepCurve(address token, LaunchedToken storage launch, BuilderPadBondingCurve curve) private {
        ...
        returns (bool)
    {
        ...
        uint256 virtualQuote = sweptQuote + BuilderPadBondingCurve(launch.curve).phantomQuote();
    ...
    function rescueCurveFees(address token) external onlyOwner nonReentrant {
        LaunchedToken storage launch = _launchedTokens[token];
        if (!launch.exists) revert TokenNotFound();
        BuilderPadBondingCurve(payable(launch.curve)).rescueFees();
    }
     ...
     *
     * Mirrors BuilderPadMemeHook.rescuePoolFees, which covers the same asset class
     ...
     * hands the exact minted assets to BuilderPadGraduationExecutor to encode
    ...
    function _createPoolAndMintPosition(
        ...
        uint160 sqrtPriceX96 = BuilderPadGraduationMath.sqrtPriceX96FromAmounts(amount0, amount1);
        ...
        memeHook.registerPool(
            ...
            BuilderPadBondingCurve(launch.curve).buybackCreatorRecipient(),
    ...
    function _requireQuotable(uint256 phantomQuote, uint256 supply, uint256 curveFeeBps) private pure {
        ...
        if (BuilderPadBondingCurveMath.quoteAmountOut(referenceBuy, phantomQuote, supply, curveFeeBps) == 0) {
