...
import {BuilderPadLaunchFactory} from "./BuilderPadLaunchFactory.sol";
import {BuilderPadBondingCurve} from "./BuilderPadBondingCurve.sol";

/**
 * @title BuilderPadLaunchAndBuy
 * @notice Creates a pons v2 launch and takes the first position on its curve
 * in a single transaction.
 *
 * BuilderPadLaunchFactory cannot fold a developer buy into `launchToken`, so a
 ...
 */
contract BuilderPadLaunchAndBuy is Ownable2Step, ReentrancyGuard {
    ...
    BuilderPadLaunchFactory public immutable factory;
    ...
    constructor(BuilderPadLaunchFactory factory_, address owner_) Ownable(owner_) {
    ...
    function launchAndBuy(
        BuilderPadLaunchFactory.TokenParams calldata params,
        ...
        if (nativeQuote) {
            tokensOut = BuilderPadBondingCurve(payable(curve)).buy{value: quoteIn}(quoteIn, minTokensOut, recipient);
        } else {
            IERC20(pairToken).forceApprove(curve, quoteIn);
            tokensOut = BuilderPadBondingCurve(payable(curve)).buy(quoteIn, minTokensOut, recipient);
