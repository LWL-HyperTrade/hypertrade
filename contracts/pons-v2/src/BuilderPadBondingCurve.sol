...
import {FeePolicySnapshot, IBuilderPadFeeEscrow, IBuilderPadFeePolicy, IBuilderPadSnipeTax} from "./interfaces/ILaunchpadV2.sol";
import {IBuilderPadLaunchFactoryGraduation} from "./interfaces/ILaunchpadV2Graduation.sol";

/**
 * @title BuilderPadBondingCurve
 ...
 */
contract BuilderPadBondingCurve is ReentrancyGuard {
    ...
    IBuilderPadFeePolicy public immutable feePolicy;
     ...
     * @param factory_ BuilderPadLaunchFactory address, the only caller allowed through `onlyFactory`.
    ...
    constructor(
        ...
        address deployer_,
        address factory_,
        IBuilderPadFeePolicy feePolicy_,
        ...
        deployer = deployer_;
        // Passed explicitly rather than read from msg.sender: BuilderPadLaunchFactory
        // deploys this curve indirectly through BuilderPadLaunchDeployer to keep its
    ...
    function initialize(address token_) external onlyFactory {
        ...
        snipeTaxStartBps = IBuilderPadSnipeTax(factory).snipeTaxStartBps();
        snipeTaxSeconds = IBuilderPadSnipeTax(factory).snipeTaxSeconds();
    ...
    /**
     * @notice Tradeable quote reserve only, matching IBuilderPadBondingCurve.
     */
    ...
    /**
     * @notice Tradeable token reserve only, matching IBuilderPadBondingCurve.
     */
        ...
        returns (uint256 tokensOut)
    {
        ...
        tokensOut = BuilderPadBondingCurveMath.getAmountOut(
        ...
        if (tokensOut > sellable) {
            ...
            uint256 net = BuilderPadBondingCurveMath.getAmountIn(sellable, quoteReserveBefore, tokenReserveBefore, 0);
            spent = Math.min(
        ...
        returns (uint256 quoteOut)
    {
        ...
        uint256 grossQuoteOut = BuilderPadBondingCurveMath.getAmountOut(tokensIn, tokenReserveBefore, quoteReserveBefore, 0);
    ...
    function _tryAutoGraduate() private {
        if (readyToGraduate()) {
            try IBuilderPadLaunchFactoryGraduation(factory).graduate(token) {}
            catch {
    ...
    function _sweepFees(uint256 minBuybackTokensOut, bool executeBuyback) private {
        ...
        if (buybackAmount != 0) {
            ...
            if (reserveMovementBps <= maxInternalPriceImpactBps) {
                ...
                uint256 tokensOut =
                    BuilderPadBondingCurveMath.quoteAmountOut(buybackAmount, quoteReserve_, tokenReserve_, 0);
     ...
     * BuilderPadFeeEscrow, and a permissioned quote asset can stop delivering to
     ...
     *
     * Mirrors BuilderPadMemeHook.rescuePoolFees for the post-graduation pool and
     * BuilderPadLaunchFactory.rescueSweptGraduation for the reserves in between.
     */
